...
#   --via-docker CONTAINER   Run every mariadb command via "docker exec -i
#                            CONTAINER mariadb ..." instead of the host client
#                            (default: unset = use host client on --host:--port).
#                            Use this on bundled VPS installs where port 3306 is
...
# Build the mariadb command as an array so we can mix host vs docker exec
# transparently. Using an array means quoting is preserved correctly for
# arguments that contain spaces (none here today, but defensive).
