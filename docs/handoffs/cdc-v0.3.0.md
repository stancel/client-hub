# Client Hub v0.3.0 — Knowledge Transfer for Complete Dental Care Website

**Audience:** the Claude Code session (or developer) working on the
**Complete Dental Care Next.js consumer site** (the one that submits
contact-form and booking-form data to
`https://client-hub-complete-dental-care.onlinesalessystems.com`).

**Status:** Client Hub v0.3.0 is **already deployed** to the CDC VPS.
Nothing on the consumer site has to change today for things to keep
working. This document explains **what changed**, **what you're
already doing right**, and **what to plan for** when paid traffic
(ads, email outreach) starts later this year.
...
**What this means for the CDC site:** **nothing**. Keep submitting
phone numbers in whatever format the form collects them. The API
handles the normalization. If you want to *display* a normalized
phone elsewhere, you can match the API's behavior using any standard
E.164 library, but it's not required.

The existing phone data on the CDC VPS was backfilled to E.164 in the
v0.3.0 migration (027). All 5 contacts on CDC now have E.164 phones.

### 2. Marketing-source attribution is now populated

Previously the `contact_marketing_sources` junction was completely
empty (zero rows on CDC). The API contract has supported a
`marketing_sources: list[str]` field on `POST /api/v1/contacts` since
v0.1.0 — but the consumer site never sent it, so the table sat
unused.
...
**Backfill:** the existing 5 contacts on CDC have been retroactively
attributed via `scripts/backfill-marketing-sources.sql`. They all
mapped to `Website` because the captured referrer was always the
consumer site's own domain (the user navigated within the site
before submitting, so `document.referrer` at submit time was a
same-domain URL — the SEO landing referrer was lost). That's the
honest answer given the data we have.

**What this means for the CDC site today:** **nothing**. Current
traffic is 100% SEO/inbound and we're not capturing first-touch UTMs
or the original landing referrer, so the API will continue to
attribute these contacts as `website`. That's correct.
...
**When you'll need it:** when CDC starts running paid ads or email
outreach (see "Future work" below). At that point the consumer site
will explicitly set `marketing_sources: ["google_search"]` (etc.) on
the POST and you can pull this list at build time to validate /
populate any UI dropdowns rather than hardcoding the codes.

## What the CDC site is already doing correctly
...
- ✅ `extra.form_name` / `extra.service_interest` — booking and contact
  form context.
...
When CDC launches paid ads, email outreach, or any non-organic
campaign — likely later this year — the consumer site should add
...
```bash
curl -sf https://client-hub-complete-dental-care.onlinesalessystems.com/openapi.json \
  | jq -r '.info.version'
# expected: 0.3.0
```
...
- File it under the consumer-site repo's `docs/` so the next session
  has it.
- Reference it when CDC's paid-traffic campaigns spin up — the
  "Future work" section is the implementation checklist.
